...
class repartissor(BaseEstimator, ClassifierMixin):
    ...
    def fit(self, X):
        ...
        self.X_ = X
        #Initialisation des plus proches voisins : 
        self.rnc.fit(self.X_[['Lat_commune', 'Lon_commune']])
    ...
    def predict(self, X):
        ...
        baseline = self.df_.neighbors_taux_de_couverture.mean()
        baseline_communes = len(self.df_[self.df_['neighbors_taux_de_couverture']<self.moy_region])
                    ...
                    while transfer==False: 
                        ...
                        for key in list(neighbors_df.index):
                            ...
                            if medecins_assignables>=1 and med_dispo>0:
                                ...
                                #trades.append([(df_.loc[ind,'code_insee'], df_.loc[key,'code_insee'], prev, df_.loc[key, 'Medecin_generaliste'])])
                                distance.append(haversine((self.df_.loc[key, 'Lat_commune'],self.df_.loc[key, 'Lon_commune']),(self.df_.loc[ind, 'Lat_commune'],self.df_.loc[ind, 'Lon_commune'])))
                            elif med_dispo>0:
        ...
        # Production du dictionnaire récapitulatif & chiffres clés
        self.df_ = preprocessing.get_meds_neighbors_df(self.df_)
        output = self.df_.neighbors_taux_de_couverture.mean()
        output_communes = len(self.df_[self.df_['neighbors_taux_de_couverture']<self.moy_region])

        #recap['trades'] = trades
        self.recap['distances'] = distance
        self.recap['scores'] = {
...
def sort_neighbors(ind, rnc, df_, sortby='distance', moy_region=0.84, radius=15, poids_des_voisins=0.1):
    ...
    Renvoie l'index de chaque voisin, avec 3 informations : 
        ...
        * Le taux de couverture (avec voisinage)
        * Le déficit en médecins (avec voisinage) (valeur positive = manque de médecin, valeur négative = surplus de médecin)
    sortby peut prendre les valeurs suivantes : 'distance', 'deficit_rate', 'deficit_absolute', 'random'
    '''
